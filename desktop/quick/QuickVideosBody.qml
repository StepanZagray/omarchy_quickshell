...
Item {
    ...
    Component.onCompleted: if (body.shell) body.shell.refreshVideos()

    property int kbdIndex: 1
    readonly property var _items: body.shell ? body.shell.videoFiles.slice(0, cols * 4) : []
    ...
    function kbdHandle(event) {
        ...
        if (k === Qt.Key_Return || k === Qt.Key_Enter || k === Qt.Key_Space) {
            if (body.kbdIndex === 0) {
                if (body.shell) body.shell.run("xdg-open "
                    + JSON.stringify(Quickshell.env("HOME") + "/Videos"));
                body.close();
                return true;
            }
            const it = body._items[body.kbdIndex - 1];
            if (it && body.shell) body.shell.copyVideoUri(it.path);
            return true;
        }
        return false;
    }
    ...
    Column {
        ...
        Item {
            ...
            Text {
                ...
                text: body.shell
                      ? body.shell.videoFiles.length + " RECENT"
                      : "—"
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    Text {
                        ...
                        text: body.shell ? body.shell.icoFilm : ""
                        color: body.root.inkDeep
                        font.family: body.root.mono
                        font.pixelSize: 22
                    }
                    Rectangle {
                        ...
                        Text {
                            id: durText
                            anchors.centerIn: parent
                            text: body.shell ? body.shell.formatVideoDuration(modelData.duration) : ""
                            color: body.root.ink
                            font.family: body.root.mono
                            font.pixelSize: 8
                        }
                    ...
                    MouseArea {
                        ...
                        onClicked: (e) => {
                            ...
                            if (e.button === Qt.RightButton)
                                body.shell.run("xdg-open " + JSON.stringify(modelData.path));
                            else
                                body.shell.copyVideoUri(modelData.path);
                        }
                    }
                }
            }
        ...
        Text {
            visible: body.shell && body.shell.videoFiles.length === 0
            width: parent.width
