...
Item {
    ...
    Component.onCompleted: if (body.shell) body.shell.refreshScreenshots()

    property int kbdIndex: 1   // 0 = CAPTURE, 1..N = thumbs
    readonly property var _items: body.shell ? body.shell.screenshotFiles.slice(0, cols * 4) : []
    ...
    function kbdHandle(event) {
        ...
        if (k === Qt.Key_Return || k === Qt.Key_Enter || k === Qt.Key_Space) {
            if (body.kbdIndex === 0) {
                if (body.shell) body.shell.run("omarchy-capture-screenshot");
                body.close();
                return true;
            }
            const it = body._items[body.kbdIndex - 1];
            if (it && body.shell) body.shell.copyScreenshotToClipboard(it.path);
            return true;
        }
        return false;
    }
    ...
    Column {
        ...
        Item {
            ...
            Text {
                ...
                text: body.shell
                      ? body.shell.screenshotFiles.length + " RECENT"
                      : "—"
                    ...
                    MouseArea {
                        ...
                        onClicked: (e) => {
                            ...
                            if (e.button === Qt.RightButton)
                                ...
                                body.shell.copyScreenshotToClipboard(modelData.path);
                        }
                    }
                }
            }
        ...
        Text {
            visible: body.shell && body.shell.screenshotFiles.length === 0
            width: parent.width
