...
Item {
    ...
    Component.onCompleted: if (body.shell) body.shell.refreshWifi()

    // 0 = RADIO toggle, 1 = SCAN, 2..N+1 = network rows.
    property int kbdIndex: 2
    readonly property int _headerCount: 2
    readonly property var _visibleNets: body.shell && body.shell.wifiRadioOn
                                        ? body.shell.wifiNetworks.slice(0, 8)
                                        : []
    ...
    function _activateAt(i) {
        body.kbdIndex = i;
        if (i === 0) { if (body.shell) body.shell.toggleWifiRadio(); return; }
        if (i === 1) { if (body.shell) body.shell.refreshWifi(); return; }
        const net = body._visibleNets[i - body._headerCount];
        if (!net || !body.shell) return;
        if (net.inUse) body.shell.disconnectWifi();
        else body.shell.connectWifi(net.ssid);
    }
    ...
    Column {
        ...
        Item {
            ...
            Text {
                ...
                text: body.shell && body.shell.wifiRadioOn
                      ? (body.shell.wifiScanning ? "SCANNING…"
                         : (body.shell.wifiNetworks.length + " NETWORKS"))
                      : "RADIO OFF"
            ...
            Row {
                ...
                QuickButton {
                    root: body.root
                    label: body.shell && body.shell.wifiRadioOn ? "RADIO OFF" : "RADIO ON"
                    selected: body.kbdIndex === 0
                    onClicked: if (body.shell) body.shell.toggleWifiRadio()
                }
                QuickButton {
                    root: body.root
                    glyph: body.shell ? body.shell.icoRefresh : ""
                    label: "SCAN"
                    selected: body.kbdIndex === 1
                    onClicked: if (body.shell) body.shell.refreshWifi()
                }
            }
        }
        ...
        Repeater {
            ...
            delegate: Rectangle {
                ...
                Text {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    text: body.shell.wifiBarsGlyph(modelData.signal)
        ...
        Text {
            visible: body.shell && body.shell.wifiRadioOn && body.shell.wifiNetworks.length === 0 && !body.shell.wifiScanning
            width: parent.width
