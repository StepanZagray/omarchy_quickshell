...
Item {
    ...
    function _launch() {
        if (body.shell) body.shell.run("omarchy-launch-or-focus-tui btop");
        body.close();
    }
    Column {
        id: col
        ...
        Repeater {
            model: [
                { label: "CPU", value: body.shell ? Math.round(body.shell.cpuVal) : 0 },
                { label: "MEM", value: body.shell ? Math.round(body.shell.memVal) : 0 }
            ]
            delegate: Item {
