...
Item {
    ...
    Column {
        ...
        Text {
            text: body.shell
                  ? Math.round(body.shell.warmthK) + "K  ·  BR " + body.shell.brightnessPct
                    + "  ·  γ " + Math.round(body.shell.gammaPct)
                    + "  ·  " + body.shell.monitorRate.toFixed(0) + "HZ"
                  : ""
        ...
        Repeater {
            ...
            delegate: DisplaySlider {
                ...
                value: body.shell ? body.shell[modelData.valKey] : 0
                minV: modelData.lo
                maxV: modelData.hi
                unit: modelData.unit
                selected: body.shell && body.shell.displayRow === modelData.row
                onCommit: function(v) {
                    if (!body.shell) return;
                    if      (modelData.row === 0) body.shell.setWarmth(v);
                    else if (modelData.row === 1) body.shell.setBrightness(v);
                    else                          body.shell.setGamma(v);
                }
                onFocusRequested: if (body.shell) body.shell.displayRow = modelData.row
            }
        }
        ...
        Item {
            ...
            Text {
                ...
                anchors.top: parent.top
                text: "PRESETS"
                color: body.shell && body.shell.displayRow === 3 ? body.root.seal : body.root.inkDeep
                font.family: body.root.mono
                font.pixelSize: 10
                font.letterSpacing: 2
                ...
                Repeater {
                    ...
                    delegate: DisplayChip {
                        ...
                        onActivated: {
                            ...
                            body.shell.applyPreset(modelData);
                        }
                    }
                }
            }
        ...
        Text {
            ...
            text: body.shell
                  ? "MONITOR · " + body.shell.monitorName + " · " + body.shell.monitorRes
                    + " · ×" + body.shell.monitorScale.toFixed(2)
                  : ""
            color: body.root.inkDeep
            font.family: body.root.mono
            font.pixelSize: 10
