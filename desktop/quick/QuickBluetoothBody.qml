...
Item {
    ...
    Component.onCompleted: if (body.shell) body.shell.refreshBluetooth()

    property int kbdIndex: 2
    readonly property int _headerCount: 2
    readonly property var _visibleDevs: body.shell && body.shell.btPowered
                                        ? body.shell.btDevices.slice(0, 8)
                                        : []
    ...
    function _activateAt(i) {
        body.kbdIndex = i;
        if (i === 0) { if (body.shell) body.shell.btTogglePower(); return; }
        if (i === 1) { if (body.shell) body.shell.btToggleScan(); return; }
        const dev = body._visibleDevs[i - body._headerCount];
        if (!dev || !body.shell) return;
        if (dev.connected) body.shell.btDisconnect(dev.mac);
        else body.shell.btConnect(dev.mac);
    }
    ...
    Column {
        ...
        Item {
            ...
            Text {
                ...
                text: !body.shell ? "—"
                      : !body.shell.btPowered ? "POWER OFF"
                      : (body.shell.btDevices.length + " DEVICES"
                         + (body.shell.btCount > 0
                            ? "  ·  " + body.shell.btCount + " CONN"
                            : "")
                         + (body.shell.btScanning ? "  ·  SCANNING" : ""))
            ...
            Row {
                ...
                QuickButton {
                    root: body.root
                    label: body.shell && body.shell.btPowered ? "POWER OFF" : "POWER ON"
                    selected: body.kbdIndex === 0
                    onClicked: if (body.shell) body.shell.btTogglePower()
                }
                QuickButton {
                    root: body.root
                    label: body.shell && body.shell.btScanning ? "SCANNING" : "SCAN"
                    selected: body.kbdIndex === 1 || (body.shell && body.shell.btScanning)
                    onClicked: if (body.shell) body.shell.btToggleScan()
                }
            }
        }
            ...
            delegate: SquircleRect {
                ...
                color: modelData.connected || kbdFocused
                       ...
                       : devMouse.containsMouse
                           ? body.root.rowHi
                           : "transparent"
                borderColor: modelData.connected || kbdFocused ? body.root.seal : body.root.sep
        ...
        Text {
            visible: body.shell && body.shell.btPowered && body.shell.btDevices.length === 0
            width: parent.width
