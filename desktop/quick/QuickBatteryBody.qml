...
Item {
    ...
    Component.onCompleted: if (body.shell) body.shell.refreshPowerProfile()

    readonly property bool hasProfile: nav && nav.powerProfile.length > 0
                                       && nav.powerProfiles.length > 0
    property int kbdIndex: 0
    readonly property int _kbdMax: hasProfile ? body.shell.powerProfiles.length : 0
    ...
    function kbdHandle(event) {
        ...
        const k = event.key;
        const n = body.shell.powerProfiles.length;
        ...
        if (k === Qt.Key_Return || k === Qt.Key_Enter || k === Qt.Key_Space) {
            const name = body.shell.powerProfiles[body.kbdIndex];
            if (name) body.shell.setPowerProfile(name);
            return true;
        }
        return false;
    }
        ...
        Text {
            ...
            text: body.shell
                  ? body.shell.batVal + "%"
                    ...
                    + (body.shell.batPower >= 0.05
                        ? "  ·  " + body.shell.batPower.toFixed(1) + "W"
                        : "")
                  : "—"
        ...
        Item {
            ...
            Column {
                ...
                Flow {
                    ...
                    spacing: 8
                    Repeater {
                        model: body.shell ? body.shell.powerProfiles : []
                        delegate: QuickButton {
                            ...
                            selected: (body.shell && body.shell.powerProfile === modelData)
                                      || body.kbdIndex === index
                            onClicked: if (body.shell) body.shell.setPowerProfile(modelData)
                        }
                    }
                }
            }
