...
Item {
    ...
    Component.onCompleted: if (body.shell) body.shell.refreshAudioSinks()

    property int kbdIndex: 1   // start on the slider — most common adjust
    readonly property int _headerCount: 2
    readonly property var _sinks: body.shell ? body.shell.audioSinks : []
    ...
    function kbdHandle(event) {
        ...
        if (body.kbdIndex === 1
            ...
            const cur = body.shell ? body.shell.audioVol : 0;
            if (body.shell) body.shell.setVolume(cur + delta);
            return true;
        }
        ...
        if (k === Qt.Key_Return || k === Qt.Key_Enter || k === Qt.Key_Space) {
            ...
            if (body.kbdIndex < body._headerCount) {
                if (body.shell) body.shell.toggleMute();
                return true;
            }
            const sink = body._sinks[body.kbdIndex - body._headerCount];
            if (sink && body.shell) body.shell.setDefaultSink(sink.id);
            return true;
        }
        return false;
    }
    ...
    Column {
        ...
        Item {
            ...
            QuickButton {
                ...
                root: body.root
                glyph: body.shell && body.shell.audioMuted
                       ? body.shell.icoMute
                       : (body.shell ? body.shell.audioIcon : "󰕾")
                label: body.shell && body.shell.audioMuted ? "UNMUTE" : "MUTE"
                selected: body.kbdIndex === 0
                onClicked: if (body.shell) body.shell.toggleMute()
            }
            QuickSlider {
                ...
                root: body.root
                value: body.shell ? body.shell.audioVol : 0
                min: 0; max: 150
                onCommitted: (v) => { if (body.shell) body.shell.setVolume(v); }
                label: body.shell && body.shell.audioMuted ? "MUTED"
                       : (body.shell ? body.shell.audioVol + "%" : "—")
            }
        }
                        ...
                        delegate: Rectangle {
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    ...
                                    if (body.shell) body.shell.setDefaultSink(modelData.id);
                                }
                            }
                        }
                    }
