...
Item {
    ...
    onModeChanged: {
        ...
        if (mode === "blueprints" && body.shell && body.shell.aetherBlueprints.length === 0)
            body.shell.refreshAetherBlueprints();
    ...
    Component.onCompleted: {
        if (body.shell && body.shell.aetherBlueprints.length === 0)
            body.shell.refreshAetherBlueprints();
    }

    // ---------- Lists per mode ----------
    readonly property var _bps: body.shell ? body.shell.aetherBlueprints.slice(0, 6) : []
    ...
    function _activateAt(i) {
        ...
        if (body.mode === "blueprints") {
            if (ci < body._bps.length) {
                if (body.shell) body.shell.applyAetherBlueprint(body._bps[ci].name);
                body.close();
            } else {
                if (body.shell) body.shell.run("sh -c 'aether --generate \"$(aether --random-wallpaper)\"'");
                body.close();
            }
            ...
            if (ci < body._themes.length) {
                ...
                if (t && body.shell) {
                    body.shell.run("sh -c 'setsid -f uwsm-app -- omarchy-theme-set \"$1\" >/dev/null 2>&1' sh "
    ...
    Column {
        ...
        Text {
            text: {
                if (body.mode === "blueprints")
                    return body.shell && body.shell.aetherLoading
                           ? "LOADING…"
                           : (body.shell
                              ? (body.shell.aetherBlueprints.length + " BLUEPRINTS")
                              : "—");
